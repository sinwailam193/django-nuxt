<template>
    <div v-if="!job" class="py-10 px-6 flex justify-center items-center">
        <LoadingSpinner />
    </div>
    <div v-else class="py-10 px-6 grid md:grid-cols-4 gap-3">
...
const { data: job } = await useLazyFetch(
    `/api/v1/jobs/${route.params.jobId}/`,
    {
        server: false,
    }
);
